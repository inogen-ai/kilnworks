...
class HeadingAwareChunker:
    ...
    def chunk(self, text: str) -> list[ChunkSpan]:
        ...
        for heading_path, body in self._split_by_headings(text):
            for piece in self._window(body):
                spans.append(ChunkSpan(text=piece, heading_path=heading_path))
        return spans

    def _split_by_headings(self, text: str) -> list[tuple[tuple[str, ...], str]]:
        ...
        def flush() -> None:
            body = "\n".join(lines).strip()
            if body:
                sections.append((tuple(title for _, title in stack), body))
            lines.clear()
